    ...
    if (status === "ready" || status === "in_progress") {
      ...
      if (order) {
        ...
        if (clientUser && clientUser.fcmToken) {
          try { await notifyClient({clientFcmToken: clientUser.fcmToken, order, title: status === "ready" ? '✅ Cordage prêt !' : '🎾 Cordage en cours !', body: status === "ready" ? `Votre raquette est prête à être récupérée !` : `Nous avons commencé le cordage de votre raquette !`}) } catch(e) { console.warn('notif client:', e) }
        }
      }
    }
  };
