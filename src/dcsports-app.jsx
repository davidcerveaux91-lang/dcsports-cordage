...
export default function App() {
  ...
  const doLogin = async () => {
    ...
    setUser(u);
    // FCM user â sauvegarder le token FCM liÃ© Ã  ce user
    let token = null; try { token = await initFCM(); } catch(e) { console.warn("FCM init failed:", e); }
    if (token) {
  ...
  const doRegister = async () => {
    ...
    if (users.find(x => x.email === regF.email)) { setAuthErr("Email dÃ©jÃ  utilisÃ©"); return; }
    ...
    setPage("account"); notify(`Compte crÃ©Ã© ! Bienvenue ${nu.name} !`);
  };
  ...
  const submitOrder = async () => {
    ...
    setPage("account"); notify("Demande envoyÃ©e ! DÃ©posez votre raquette au magasin.");
  };
  ...
  const updateStatus = async (id, status) => {
    ...
    if (status === "ready") {
      ...
      notify(`ð Notification envoyÃ©e Ã  ${order.userName}`);
    }
  };
